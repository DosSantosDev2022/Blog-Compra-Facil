...
interface UpdateArticleViewCountResponse {
  ...
  publishArticle: { // Adicionando o tipo para a resposta da publicação
    id: string;
    stage: string;
  };
}
...
export const updateViewCount = async (
  ...
  try {
    const data = await HygraphMutation<UpdateArticleViewCountResponse>(mutation, variables);
    console.log(`Contagem de visualização do artigo com ID ${id} atualizada e republicada com sucesso.`);
    return data;
  } catch (error) {
    console.error('Erro ao atualizar e republicar a contagem de visualizações do artigo:', error);
    return null;
  }
};
...
describe('getDetailsArticle', () => {
  ...
  it('should throw an error if HygraphQuery fails', async () => {
    (HygraphQuery as Mock).mockRejectedValueOnce(new Error('Hygraph error'));

    await expect(getDetailsArticle('broken-slug')).rejects.toThrow('Hygraph error');
  });
  ...
  it('should search with the correct slug variable', async () => {
    ...
    expect(HygraphQuery).toHaveBeenCalledWith(expect.any(String), { slug }, expect.any(Object));
